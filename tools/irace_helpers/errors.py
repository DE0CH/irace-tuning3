...
def check_numbers(start, end, log):
    ...
    if log:
        if (type(start) is int or float) and start <= 0 or (type(end) is int or float) and end <= 0:
            irace_assert(False, "Domain of type 'log' cannot be non-positive")

def check_illegal_character(name):
    irace_assert(re.match("^[_\-a-zA-Z0-9]+$", name), f"name {repr(name)} container illegal character. THe only allowed characters are a-z, A-Z, 0-9, _ (underscore) and -.")
