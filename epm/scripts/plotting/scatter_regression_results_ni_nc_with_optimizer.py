...
def main():
    ...
    enc_mat, inst_feat_dict, inst_ls, n_feats_used, perf_ls, to_ls, cen_ls, id_ls = \
        load_data(args.target_algo_runs, feature_file=args.feature_file,
                  ...
                  oneHot=False, par=args.par, downsample=DOWNSAMPLE, return_file_id=True)
    ...
    for grid_idx, mode in enumerate(["train", "both"]):
        ...
        #fig.suptitle("%s" % os.path.basename(args.pkl).replace("_", " "), fontsize=13)
        ...
        for ls_idx, b_idx in enumerate(bool_array):
            ...
            color_ls = [colors[i] for i in file_ind]
            if ls_idx == len(bool_array) - 1:
                ax.scatter(x=real, y=pred, facecolor=color_ls,
                           c='', edgecolor="", alpha=0.6, s=4)
            else:
                pass
        ...
        if args.save is not None:
            ...
            save_fn = args.save + "_%s_file_scatter" % mode
