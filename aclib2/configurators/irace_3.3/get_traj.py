    ...
    with open(fn_in) as fp:
        while True:
            ...
            if line.startswith("Description of the best-so-far configuration:"):
                ...
                config = ["%s='%s'" % (name_mapping[k], v.strip()) for k, v in zip(params, config) if v != "<NA>" and v != "NA"]

                if wc_time is not None and cost is not None and target_time is not None:
                    add_to_traj = [target_time, cost, wc_time, id, 0]
